...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   ////	 Macro for Test Type(Chinese：测试类型宏定义)
	 #define TEST_M8266WIFI_TYPE    7	     //           1 = Repeative Sending, 2 = Repeative Reception, 3 = Echo  4 = multi-clients transimission test
...
//		 }
	...
	else  // else: setup connection successfully, we could config it (Chinese: 创建套接字成功，就可以配置套接字）
	{
		 ...
		 if(M8266WIFI_SPI_Op_Multicuast_Group(0, "224.6.6.6", &status)==0)
     {
		 ...
		 if( M8266WIFI_SPI_Set_TcpServer_Auto_Discon_Timeout(link_no, 60000, &status) == 0)
     {
		   while(1)
		   {
